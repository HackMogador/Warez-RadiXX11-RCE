...
procedure Blowfish_Encrypt(const InData; var OutData);
procedure Blowfish_Init(const Key; Size: Integer);

implementation

const
  PBoxOrg: array[0..17] of LongWord= (
    ...
    $9216d5d9, $8979fb1b);
  SBoxOrg: array[0..3,0..255] of LongWord= ((
...
begin
  xL := PLongWord(@InData)^;
  xR := PLongWord(LongWord(@InData) + 4)^;
  ...
  begin
    ...
    L := (xL and $FF00) shr 8;
    H := xL and $FF;
    J := xR xor ((J xor SBox[2, L]) + SBox[3, H]);
    xR := xL;
    xL := J;
  end;
  ...
  xL := xL xor PBox[16];
  xR := xR xor PBox[17];
  PLongWord(LongWord(@OutData) + 4)^ := xL;
...
begin
  ...
  begin
    ...
    Move(Key, Buffer, Size);

    for I := 0 to SizeOf(Buffer) - Size - 1 do
      Buffer[Size + I] := Buffer[I];

    for I := 17 downto 0 do
      PBox[I] := PBox[I] xor BSwap(PInteger(@Buffer[I * 4])^);

    FillChar(Buffer, 8, 0);
    P1 := @Buffer;
    P2 := @PBox;

    for I := 1 to SizeOf(PBox) div 8 do
    begin
    ...
    for I := 1 to SizeOf(SBox) div 8 do
    begin
      Blowfish_Encrypt(P1^, P2^);
      P1 := P2;
      P2 := Pointer(LongWord(P2) + 8);
