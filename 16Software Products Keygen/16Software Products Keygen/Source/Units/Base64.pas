...
function Base64Encode(const S: AnsiString): AnsiString; overload;
function Base64URLEncode(const Data: PAnsiChar; const DataSize: integer): AnsiString; overload;
function Base64URLEncode(const S: AnsiString): AnsiString; overload;
...
begin
  Result := Base64Decode(PChar(S), Length(S));
end;
...
begin
  Result := Base64Encode(PChar(S), Length(S));
end;
...
begin
  Result := Base64URLEncode(PChar(S), Length(S));
end;
